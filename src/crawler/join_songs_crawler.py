...
try:
    for j in pageNums:
        url = f"https://www.melon.com/artist/songPaging.htm?startIndex={j}&pageSize=50&listType=F&orderBy=ISSUE_DATE&artistId=786648"
        songs = requests.get(url, headers=headers, timeout=5)
    ...
    # 곡 상세정보 추가
    for i in range(len(JoinSongsData)):
        ...
        JoinSongsData[i]["release"] = release
